...
class Heap {
 public:
  class iterator {
    ...
    iterator(Heap<T, C>& _heap, int _index) : heap(_heap), index(_index) {
      // empty
    }

   public:
    iterator(iterator&& other) : heap(other.heap), index(other.index) {
      // empty
    }
    ...
    T& operator*() { return heap.data[index]; }

    // the item this iterator refers to
    void increase() { heap.siftUp(index); }
  };  // class iterator
