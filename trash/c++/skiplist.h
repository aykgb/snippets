...
typename SkipList<Key, Comparator>::Node* SkipList<Key, Comparator>::NewNode(
    ...
    return new (new_mem) Node(
        key);  // 使用自己分配的内存空间上调用构造函数,没有考虑资源的释放，资源泄露

    // return static_cast<Node*>(malloc(sizeof(Node) + sizeof(void*) * (height -
    // 1))); //
    // 没有调用构造函数，malloc只分配空间，不会调用构造函数。而new会调用构造函数
}
