...
/**                                                                    **/
/**********************************END***********************************/
OSM2_HAL_OAM::PSM_HAL_OAM(unsigned int equip_max) : HAL_OAM(equip_max), isShutdownWarm(false), isBootWarm(false)
{
    //initiate DB
...
/**                                                                    **/
/**********************************END***********************************/
void OSM2_HAL_OAM::InitWarm(void)
{
    HAL_INFO_PRINT("OSM2_HAL_OAM: InitWarm() executing");
    isBootWarm = true;
...
//  }
    ...
    HAL_INFO_PRINT("OSM2_HAL_OAM: InitWarm() complete");
...
/**                                                                    **/
/**********************************END***********************************/
void OSM2_HAL_OAM::ShutdownWarm(void)
{
    HAL_INFO_PRINT("OSM2_HAL_OAM: ShutDownWarm() executing");
    ...
    HAL_INFO_PRINT("OSM2_HAL_OAM: ShutDownWarm() complete");
}
...
/**                                                                    **/
/**********************************END***********************************/
HAL_ERR OSM2_HAL_OAM::StoreDb(string filename)
{
//    int i;
...
//    BackupDb->version = 1;
    return OSM2_HAL_OAM_OPE_DB_API_INST->StoreDb(filename);
}
...
/**                                                                    **/
/**********************************END***********************************/
HAL_ERR OSM2_HAL_OAM::RestoreDb(string filename)
{
//    int i;
//    HAL_PKT_DB_1_0 *BackupDb;
    return OSM2_HAL_OAM_OPE_DB_API_INST->RestoreDb(filename);
}
...
/**                                                                    **/
/**********************************END***********************************/
HAL_ERR OSM2_HAL_OAM::MEP_Create(OAMHANDLE *handle, MEP_INFO *mep_info)
{
    ...
    if(mep_info->header.mp_type == e_ME) {
        ...
        OSM2_HAL_OAM_wb_me_key key(cfg_data->u.enet.context_index, cfg_data->u.enet.md_index, cfg_data->u.enet.ma_index);

        if(FPAL_SOAM_IS_ENET_SERVICE(mp_tech_type)) {
            OSM2_HAL_OAM_ope_entry_me* entry = NULL;
            if( OSM2_HAL_OAM_OPE_DB_API_INST->get_me_map_entry(key, entry) == HAL_SUCCESS) {
                //fill in the handle and return
                handle->type = OAMHANDLE_ME;
                handle->id = entry->get_hal_id();

                OSM2_HAL_OAM_OPE_DB_UNLOCK();
        ...
        if(HAL_ERROR == OSM2_HAL_OAM_OPE_DB_API_INST->take(
        ...
        if(me_oper(e_create, (my_handle->id),
            ...
            OSM2_HAL_OAM_OPE_DB_API_INST->give(e_ME,
                                               (my_handle->id));
        ...
        *handle = *my_handle;
        ...
        OSM2_HAL_OAM_wb_mep_key key(tmp_me_hal_id, cfg_data->mep_cp_id);
        OSM2_HAL_OAM_ope_entry_mep* entry = NULL;
        if( OSM2_HAL_OAM_OPE_DB_API_INST->get_mep_map_entry(key, entry) == HAL_SUCCESS) {
        ...
        if(mep_oper(e_create, (my_handle->id),
                    mep_info) != HAL_SUCCESS) {
            OSM2_HAL_OAM_OPE_DB_API_INST->give(e_MEP,
                                               (my_handle->id));
...
/**                                                                    **/
/**********************************END***********************************/
HAL_ERR OSM2_HAL_OAM::MEP_Modify(OAMHANDLE *handle, MEP_INFO *mep_info)
{
    //validate input pointers
    if((handle == NULL) || (mep_info == NULL) || (mep_info->buffer == NULL)) {
        UTIL_ERR_MSG_INVALID_INPUT();
...
/**                                                                    **/
/**********************************END***********************************/
HAL_ERR OSM2_HAL_OAM::MEP_Delete(OAMHANDLE *handle, MEP_INFO *mep_info)
{
    //validate input pointers
    if(handle == NULL) {
        UTIL_ERR_MSG_INVALID_INPUT();
    ...
    if(OSM2_HAL_OAM_EXPERT_TRC_INFO_ON) {
    ...
    if(mep_info->header.mp_type == e_ME) {
        ...
        if(OSM2_HAL_OAM_OPE_DB_API_INST->give(e_ME, id) != HAL_SUCCESS) {
            OSM2_HAL_OAM_OPE_DB_UNLOCK();
        ...
        if(OSM2_HAL_OAM_OPE_DB_API_INST->give(e_MEP, id) != HAL_SUCCESS) {
            OSM2_HAL_OAM_OPE_DB_UNLOCK();
            return HAL_ERROR;
        }
...
/**                                                                    **/
/**********************************END***********************************/
HAL_ERR OSM2_HAL_OAM::MEP_Dump(OAMHANDLE *handle, MEP_INFO *mep_info)
{
    //validate input pointers
    if(handle == NULL) {
        UTIL_ERR_MSG_INVALID_INPUT();
...
/**                                                                    **/
/**********************************END***********************************/
HAL_ERR OSM2_HAL_OAM::RMEP_Create(OAMHANDLE *handle, RMEP_INFO *rmep_info)
{
    OAMHANDLE* my_handle = NULL;
    ...
    OSM2_HAL_OAM_wb_rmep_key key(cfg_data->mep_hal_id, tmp_rmep_id);

    OSM2_HAL_OAM_ope_entry_rmep* entry = NULL;

    if( OSM2_HAL_OAM_OPE_DB_API_INST->get_rmep_map_entry(key, entry) == HAL_SUCCESS) {
    ...
    if(rmep_oper(e_create, (my_handle->id),
        ...
        OSM2_HAL_OAM_OPE_DB_API_INST->give(e_RMEP,
                                           (my_handle->id));
...
/**                                                                    **/
/**********************************END***********************************/
HAL_ERR OSM2_HAL_OAM::RMEP_Modify(OAMHANDLE *handle, RMEP_INFO *rmep_info)
{
    //validate input pointers
    if((handle == NULL) || (rmep_info == NULL) || (rmep_info->buffer == NULL)) {
        UTIL_ERR_MSG_INVALID_INPUT();
...
/**                                                                    **/
/**********************************END***********************************/
HAL_ERR OSM2_HAL_OAM::RMEP_Delete(OAMHANDLE *handle, RMEP_INFO *rmep_info)
{
    //validate input pointers
    if(handle == NULL) {
        UTIL_ERR_MSG_INVALID_INPUT();
    ...
    if(cfg_data->header.b_process_in_hal == true) {
        if(OSM2_HAL_OAM_OPE_DB_API_INST->give(e_RMEP,
...
/**                                                                    **/
/**********************************END***********************************/
HAL_ERR OSM2_HAL_OAM::RMEP_Dump(OAMHANDLE *handle, RMEP_INFO *rmep_info)
{
    //validate input pointers
    if(handle == NULL) {
        UTIL_ERR_MSG_INVALID_INPUT();
...
/**                                                                    **/
/**********************************END***********************************/
HAL_ERR OSM2_HAL_OAM::MIP_Create(OAMHANDLE *handle, MIP_INFO *mip_info)
{
    OAMHANDLE* my_handle = NULL;
    ...
    OSM2_HAL_OAM_wb_mip_key key(cfg_data->lif_id, cfg_data->u.enet.md_level);
    OSM2_HAL_OAM_ope_entry_mip* entry = NULL;

    if(FPAL_SOAM_IS_ENET_SERVICE(mp_tech_type)) {
        if( OSM2_HAL_OAM_OPE_DB_API_INST->get_mip_map_entry(key, entry) == HAL_SUCCESS) {
    ...
    if(mip_oper(e_create, (my_handle->id),
        ...
        OSM2_HAL_OAM_OPE_DB_API_INST->give(e_MIP,
                                           (my_handle->id));
...
/**                                                                    **/
/**********************************END***********************************/
HAL_ERR OSM2_HAL_OAM::MIP_Modify(OAMHANDLE *handle, MIP_INFO *mip_info)
{
    //validate input pointers
    if((handle == NULL) || (mip_info == NULL) || (mip_info->buffer == NULL)) {
        UTIL_ERR_MSG_INVALID_INPUT();
...
/**                                                                    **/
/**********************************END***********************************/
HAL_ERR OSM2_HAL_OAM::MIP_Delete(OAMHANDLE *handle, MIP_INFO *mip_info)
{
    //validate input pointers
    if(handle == NULL) {
        UTIL_ERR_MSG_INVALID_INPUT();
    ...
    if(OSM2_HAL_OAM_OPE_DB_API_INST->give(e_MIP,
                                          (handle->id))) {
        OSM2_HAL_OAM_OPE_DB_UNLOCK();
        UTIL_ERR_MSG("give FAILED...");
...
/**                                                                    **/
/**********************************END***********************************/
HAL_ERR OSM2_HAL_OAM::MIP_Dump(OAMHANDLE *handle, MIP_INFO *mip_info)
{
    //validate input pointers
    if(handle == NULL) {
        UTIL_ERR_MSG_INVALID_INPUT();
...
/**                                                                    **/
/**********************************END***********************************/
HAL_ERR OSM2_HAL_OAM::MEP_Create_AIS_Client(OAMHANDLE *handle,
        MEP_AIS_CLIENT_INFO *mep_ais_client_info)
{
...
#ifdef __SIMULATION__
    if(OSM2_HAL_OAM_EXPERT_TRC_VERBOSE_INFO_ON) {
    ...
    OSM2_HAL_OAM_wb_fm_key key(cfg_data->context_index, cfg_data->md_index, cfg_data->ma_index, cfg_data->mep_id);
    OSM2_HAL_OAM_ope_entry_fm* entry = NULL;

    if( OSM2_HAL_OAM_OPE_DB_API_INST->get_fm_map_entry(key, entry) == HAL_SUCCESS) {
    ...
    if(fm_client_oper(e_create, e_AIS,
        ...
        OSM2_HAL_OAM_OPE_DB_API_INST->give(e_FM_CLIENT,
                                           (my_handle->id));
...
/**                                                                    **/
/**********************************END***********************************/
HAL_ERR OSM2_HAL_OAM::MEP_Modify_AIS_Client(OAMHANDLE *handle,
        MEP_AIS_CLIENT_INFO *mep_ais_client_info)
{
    //validate input pointers
    if((handle == NULL) || (mep_ais_client_info == NULL) ||
...
/**                                                                    **/
/**********************************END***********************************/
HAL_ERR OSM2_HAL_OAM::MEP_Delete_AIS_Client(OAMHANDLE *handle,
        MEP_AIS_CLIENT_INFO *mep_ais_client_info)
{
    //validate input pointers
    if(handle == NULL) {
    ...
    if(OSM2_HAL_OAM_OPE_DB_API_INST->give(e_FM_CLIENT,
                                          (handle->id)) != HAL_SUCCESS) {
        OSM2_HAL_OAM_OPE_DB_UNLOCK();
        UTIL_ERR_MSG("give FAILED...");
        return HAL_ERROR;
...
/**                                                                    **/
/**********************************END***********************************/
HAL_ERR OSM2_HAL_OAM::MEP_Create_LCK_Client(OAMHANDLE *handle,
...
/**                                                                    **/
/**********************************END***********************************/
HAL_ERR OSM2_HAL_OAM::MEP_Modify_LCK_Client(OAMHANDLE *handle,
        MEP_LCK_CLIENT_INFO *mep_lck_client_info)
{
    //validate input pointers
    if((handle == NULL) || (mep_lck_client_info == NULL) ||
...
/**                                                                    **/
/**********************************END***********************************/
HAL_ERR OSM2_HAL_OAM::MEP_Delete_LCK_Client(OAMHANDLE *handle,
        MEP_LCK_CLIENT_INFO *mep_lck_client_info)
{
    //validate input pointers
    if(handle == NULL) {
...
//        Notes:
//-----------------------------------------------------------------------------
HAL_ERR OSM2_HAL_OAM::MEP_CC_Events_Set(OAMHANDLE *handle,
...
/**                                                                    **/
/**********************************END***********************************/
HAL_ERR OSM2_HAL_OAM::SOAM_Pdu_Xmt(OAMHANDLE *handle, MEP_PDU_TX *pdu,
                                   unsigned int size)
{
    (void)size;
    ...
    return OSM2_HAL_OAM_TX_FRAME_HANDLER_INST->send_frame_request(handle->id, pdu);
}
...
/**                                                                    **/
/**********************************END***********************************/
HAL_ERR OSM2_HAL_OAM::SOAM_Pdu_Get(OAMHANDLE *handle, MEP_PDU_RX *input_pdu,
                                   MEP_PDU_RX *output_pdu, unsigned int *ret_size)
{
    ...
    *ret_size = 0;
    ...
    if(input_pdu->header.pdu_type == e_unknown_pdu) {
        ...
        if(OSM2_HAL_oam_unknown_pkt_handler::Instance()->get_pkt(handle->id,
...
/**                                                                    **/
/**********************************END***********************************/
HAL_ERR OSM2_HAL_OAM::MEP_Debug_Counts_Get(OAMHANDLE *handle,
...
/**                                                                    **/
/**********************************END***********************************/
HAL_ERR OSM2_HAL_OAM::MEP_BFD_session_data_Get(OAMHANDLE *handle,
...
/**                                                                    **/
/**********************************END***********************************/
HAL_ERR OSM2_HAL_OAM::global_info_set(HAL_OAM_GLOBAL_INFO* global_info)
{
    OSM2_HAL_OAM_OPE_DB_LOCK();
...
/**                                                                    **/
/**********************************END***********************************/
void* OSM2_HAL_OAM::MonTsk(void *pArg)
{
    (void)pArg;
    void *ret = NULL;
...
//        Notes:
//-----------------------------------------------------------------------------
HAL_ERR OSM2_HAL_OAM::me_oper(int8 oper, uint32 hal_id, MEP_INFO* mep_info)
{
    OSM2_HAL_OAM_ope_entry_me* entry = NULL;

    uint16 id = (hal_id);

    if(NULL == (entry = (OSM2_HAL_OAM_ope_entry_me*)(PSM_HAL_OAM_OPE_DB_API_INST->get_entry(e_ME, id)))) {
...
//        Notes:
//-----------------------------------------------------------------------------
HAL_ERR OSM2_HAL_OAM::mep_oper(int8 oper, uint32 hal_id, MEP_INFO* mep_info)
{
    OSM2_HAL_OAM_ope_entry* tmp_entry = NULL;
    OSM2_HAL_OAM_ope_entry_mep* entry = NULL;
    uint16 outer_vlan;
    uint16 inner_vlan;
    uint8  outer_tpid;
    ...
    if((oper == e_create) || (oper == e_modify) || (oper == e_delete)) {
        if(OSM2_HAL_OAM_TRC_VERBOSE_INFO_ON||PSM_HAL_OAM_EXPERT_TRC_VERBOSE_INFO_ON) {
    ...
    if(NULL == (tmp_entry = OSM2_HAL_OAM_OPE_DB_API_INST->get_entry(e_MEP, id))) {
        UTIL_ERR_MSG("unable to get MEP entry in DB,oper=%d,id=%d",oper,id);
        return HAL_ERROR;
    }

    entry = reinterpret_cast<OSM2_HAL_OAM_ope_entry_mep*>(tmp_entry);
    ...
    if((oper == e_create) || (oper == e_modify)) {
...
#if 0
            OSM2_HAL_oam_ope_port* port_entry = PSM_HAL_OAM_OPE_DB_API_INST->
                                                get_port_entry(
        ...
        if(oper == e_create) {
            ...
            if(FPAL_SOAM_IS_ENET_SERVICE(mp_tech_type)) {
                if(OSM2_HAL_OAM_OPE_DB_API_INST->add_mac_port_map_entry(
                ...
                if(OSM2_HAL_OAM_OPE_DB_API_INST->add_lif_to_port_map_entry(
...
#endif
        if((OSM2_HAL_OAM_TRC_INFO_ON) || (PSM_HAL_OAM_EXPERT_TRC_INFO_ON)) {
...
//        Notes:
//-----------------------------------------------------------------------------
HAL_ERR OSM2_HAL_OAM::rmep_oper(int8 oper, uint32 hal_id, RMEP_INFO* rmep_info)
{
    OSM2_HAL_OAM_ope_entry* tmp_entry = NULL;
    OSM2_HAL_OAM_ope_entry_rmep* entry = NULL;
    ...
    if(NULL == (tmp_entry = OSM2_HAL_OAM_OPE_DB_API_INST->get_entry(e_RMEP, id))) {
        UTIL_ERR_MSG("unable to get RMEP entry in DB,oper=%d,id=%d",oper,id);
        return HAL_ERROR;
    }

    entry = reinterpret_cast<OSM2_HAL_OAM_ope_entry_rmep*>(tmp_entry);
    ...
    if((oper == e_create) || (oper == e_modify)) {
        ...
        if(oper == e_create) {
            ...
            if(rmep_count == 1) {
                ...
                int arad_oam_id =
                    OSM2_HAL_OAM_OPE_DB_API_INST->get_rmep_arad_id_for_rmep_in_arad(
                        (cfg_data->mep_hal_id));
    ...
    //delete the entity
        ...
        if(entry->get_in_arad()) {
            ...
            if(rmep_count > 1) {
                ...
                OSM2_HAL_OAM_ope_entry* tmp_mep_entry = NULL;

                if(NULL == (tmp_mep_entry = OSM2_HAL_OAM_OPE_DB_API_INST->get_entry(e_MEP, cfg_data->mep_hal_id))) {
                    UTIL_ERR_MSG("unable to find entry");
                    return HAL_ERROR;
                }

                OSM2_HAL_OAM_ope_entry_mep* mep_entry =
                    reinterpret_cast<OSM2_HAL_OAM_ope_entry_mep*>(tmp_mep_entry);

                vector<OSM2_HAL_OAM_ope_entry_rmep*> rmeps = mep_entry->get_rmeps();

                //find the RMEP which is different from the one being deleted
                vector<OSM2_HAL_OAM_ope_entry_rmep*>::iterator it;
...
//        Notes:
//-----------------------------------------------------------------------------
HAL_ERR OSM2_HAL_OAM::fm_client_oper(int8 oper, int8 fm_type,
                                     uint32 hal_id, MEP_AIS_CLIENT_INFO* fm_client_info)
{
    ...
    if((oper == e_create) || (oper == e_modify)) {
        ...
        if(FPAL_SOAM_IS_ENET_SERVICE(fm_client_info->header.mp_tech_type)) {
            ...
            if(OSM2_HAL_OAM_TRC_VERBOSE_INFO_ON||
                    OSM2_HAL_OAM_EXPERT_TRC_VERBOSE_INFO_ON) {
            ...
            if(NULL == (entry = (OSM2_HAL_OAM_ope_entry_fm*)(PSM_HAL_OAM_OPE_DB_API_INST->get_entry(e_FM_CLIENT, id)))) {
        ...
        if(OSM2_HAL_OAM_EXPERT_TRC_FPGA_INFO_ON||PSM_HAL_OAM_EXPERT_TRC_VERBOSE_INFO_ON) {
...
//        Notes:
//-----------------------------------------------------------------------------
HAL_ERR OSM2_HAL_OAM::mip_oper(int8 oper, uint32 hal_id, MIP_INFO* mip_info)
{
    OSM2_HAL_OAM_ope_entry* tmp_entry = NULL;
    OSM2_HAL_OAM_ope_entry_mip* entry = NULL;
    ...
    if(NULL == (tmp_entry = OSM2_HAL_OAM_OPE_DB_API_INST->get_entry(e_MIP, id))) {
        UTIL_ERR_MSG("unable to get MIP entry in DB,oper=%d,id=%d",oper, id);
        return HAL_ERROR;
    }
    entry = reinterpret_cast<OSM2_HAL_OAM_ope_entry_mip*>(tmp_entry);
...
/**                                                                    **/
/**********************************END***********************************/
HAL_ERR OSM2_HAL_OAM::sync_info_set(HAL_OAM_SYNC_INFO* sync_info)
{
    //(void*) sync_info;
...
/**                                                                    **/
/**********************************END***********************************/
HAL_ERR OSM2_HAL_OAM::rfc2544_warm_init_data_get(uint16 *lbrIndex,
...
/**                                                                    **/
/**********************************END***********************************/
HAL_ERR OSM2_HAL_OAM::rfc2544_warm_init_data_set(uint16 lbrIndex,
        struct Devices::Fpga::rfc2544State *state,
        HAL_OAM_EVENT_DATA  *oam_ev)
{
